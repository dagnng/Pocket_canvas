...
function setupToolbox() {
 ...
 // set up canvas tools 
  ...
  // set up background style tools 
  const backgroundStyles = select('section.toolbox div.background-tools')
  setupBgColorPicker(backgroundStyles)
  // set up paint slide tools 
  const paintSlide = select('section.toolbox div.slide-tools')
  setupOpacitySlider(paintSlide)
  setupBrushSizeSlider(paintSlide)
}
...
function setupBrushSelector(parentTag) {
  ...
  makeLabel(brushSelector, parentTag, 'Brushes')
...
function draw() {
  ...
  if (mouseIsPressed && mouseX <= windowWidth) {
    // set the paint color
    setPaintColor()
